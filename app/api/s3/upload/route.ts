...
export async function POST(request: Request) {
  try {
    ...
    // Create command for upload (bucket policy handles public access)
    ...
    // Build the public S3 URL for preview (AWS S3 virtual-hosted style)
    const region = env.AWS_REGION || "eu-north-1";
    const publicUrl = `https://${env.NEXT_PUBLIC_S3_BUCKET_NAME_IMAGES}.s3.${region}.amazonaws.com/${uniqueKey}`;
